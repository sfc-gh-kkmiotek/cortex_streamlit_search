...
search_service_popular = get_service("search_service_popular_2")
search_service_unpopular = get_service("search_service_un_popular_2")
...
def search(query, filters=None, order_by="default"):
    resp_popular = search_service_popular.search(
        query=query,
        filter=filters,
        columns=["app_id", "title", "unique_views"],
        limit=20
    )
